...
class S3Events(BaseEventHandler, WithLogging):
    ...
    def __init__(self, charm: CharmBase, context: Context, workload: ConfigurationHubWorkloadBase):
        ...
        self.pushgateway = PrometheusPushgatewayRequirer(self.charm)

        self.framework.observe(self.pushgateway.on.relation_changed, self._on_pushgateway_changed)
        self.framework.observe(self.pushgateway.on.relation_broken, self._on_pushgateway_gone)
    ...
    def _on_pushgateway_gone(self, _: RelationBrokenEvent):
        ...
        self.charm.unit.status = self.get_app_status(None)
        if self.charm.unit.is_leader():
            self.charm.app.status = self.get_app_status(None)
